...
Run this to clean up closed events
"""
...
def main():
    ...
    # Use db_manager to fetch data
    total = events_mgr.db_manager.fetch_one("SELECT COUNT(*) as count FROM events")
    closed = events_mgr.db_manager.fetch_one("SELECT COUNT(*) as count FROM events WHERE closed = 1")
    active = events_mgr.db_manager.fetch_one("SELECT COUNT(*) as count FROM events WHERE closed = 0")
    ...
    remaining = events_mgr.db_manager.fetch_one("SELECT COUNT(*) as count FROM events")
    print(f"   Deleted: {deleted:,} closed events")
    print(f"   Remaining: {remaining['count']:,} active events")
    
    # Vacuum database to reclaim space
    print("\n🔧 Optimizing database...")
    events_mgr.db_manager.vacuum()
